		...
		if world_state_buffer.size() > 2: # we have a future state so we should interpolate
			...
			for player in world_state_buffer[2].keys():
				...
				if get_node("/root/MainScene/OtherPlayers").has_node(str(player)):
					...
					Server.SpawnNewPlayer(player, world_state_buffer[2][player]["P"])
		elif render_time > world_state_buffer[1].T: # we do not have a future state so we should extrapolate if 
			var extrapolation_factor = float(render_time - world_state_buffer[1]["T"]) / float(world_state_buffer[2]["T"] - world_state_buffer[1]["T"]) - 1.0
			for player in world_state_buffer[2].keys():
				if str(player) == "T":
					continue
				if player == get_tree().get_network_unique_id():
				...
				if not world_state_buffer[1].has(player):
					continue
				if get_node("/root/MainScene/OtherPlayers").has_node(str(player)):
					var position_delta =  world_state_buffer[1][player]["P"] - world_state_buffer[2][player]["P"]
